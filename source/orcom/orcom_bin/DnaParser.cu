...
#ifdef CUDA
	extern uint64 h_dnaSize;
	extern char* h_reads;
	extern uint16* h_lenReads;	
#endif
...
uint64 DnaParser::ParseFrom(const DataChunk& chunk_, DataChunk& dnaBuffer_, std::vector<DnaRecord>& records_, uint64& rec_count_)
{
	...
	rec_count_ = 0;
	while (memoryPos < memorySize && ReadNextRecord(records_[rec_count_]))	// Pone cada Read en un Objeto
	{
		rec_count_++;
...
#ifdef CUDA
	h_reads = (char*)malloc(dnaSize * sizeof(char));
	h_dnaSize = dnaSize;
	h_lenReads = (uint16*)malloc(rec_count_ * sizeof(uint16));
	memcpy(h_reads, dnaMemory, dnaSize);
#endif
